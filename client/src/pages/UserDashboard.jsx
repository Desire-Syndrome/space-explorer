				...
				<div className='border-r-2 max-w-1/4 border-b-2 md:border-hidden'>
					<ul className='flex flex-col items-start pt-5 pb-2 text-gray-800 text-sm lg:text-base'>
						...
						<li className='w-full mt-2 border-t-2'>
							<NavLink to={'/user/bookmarks/picture-of-the-day'} className={({ isActive }) => `flex items-center p-3 md:px-6 gap-2 w-full  mt-2 ${isActive && 'bg-blue-100'}`} >
						...
						<li className='w-full'>
							<NavLink to={'/user/bookmarks/earth-from-space'} className={({ isActive }) => `flex items-center p-3 md:px-6 gap-2 w-full ${isActive && 'bg-blue-100'}`} >
								<img src={assetsImages.bookmark} className='w-5' alt="icon" />
