...
const NasaEpicCard = ({ item, userInfo, bookmarks }) => {
  ...
  useEffect(() => {
    const alreadyAdded = bookmarks.some(
      (bookmark) => bookmark.type === "epic" && bookmark.data?.id === item.id
    );
  ...
  const handleBookmarkClick = () => {
    if (userInfo) {
      if (!isBookmarked) {
        ...
        dispatch(addBookmarkAction("epic", bookmarkData));
      } else {
  ...
  return (<>
    ...
    <div className="border rounded shadow p-2">
      <div onClick={loadedImage ? () => setShowModal(true) : undefined} className="w-full h-58 md:h-80 xl:h-96 bg-white flex justify-center items-center overflow-hidden rounded mb-2 relative">
        {!loadedImage && (
          <img src={assetsImages.loadingImage} alt="Loading image" className="w-full h-full object-cover absolute" />
        )}
        <img src={item.image} alt={item.caption} className={`w-full h-full object-cover cursor-pointer hover:scale-105 transition duration-300
          ${loadedImage ? "opacity-100" : "opacity-0"}`} onLoad={() => setLoadedImage(true)} />
      </div>

      <p className="text-sm md:text-base font-semibold">{item.caption}</p>
      <p className="mt-1 text-sm md:text-base text-gray-600">{item.date}</p>
    ...
    {showModal && (
      <div onClick={() => setShowModal(false)} className="fixed inset-0 bg-black bg-opacity-95 flex justify-center items-center z-50">
        ...
        <div className="relative px-2" onClick={(e) => e.stopPropagation()}>
          <img src={item.image} alt={item.caption} className="w-full max-h-[90vh]" />
        </div>
      </div>
    )}
